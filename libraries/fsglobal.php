	...
	function get_menu_by_group($group_id){
		if(USE_MEMCACHE){
			...
			if($menu_in_memcache){
				...
				global $db;
			$sql = " SELECT id,link, name, level, parent_id as parent_id,image, target,group_id,description,icon,image,is_rewrite,width_icon,heigth_icon
					FROM fs_menus_items
					WHERE published  = 1
						ORDER BY ordering";
				$db->query($sql);
			...
			global $db;
			$sql = ' SELECT id,link, name,image, level, parent_id as parent_id, target,group_id, description,icon,image,is_rewrite,width_icon,heigth_icon
				FROM fs_menus_items
